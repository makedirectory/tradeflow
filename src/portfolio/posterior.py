"""Black–Litterman posterior (Spec 021): blend our views with Σ's implied propagation.

017 backs out the consensus (reverse-optimized) expected returns for which the
benchmark is itself optimal; our alphas are *deviations from* that consensus. Left
alone, the optimizer reads a name with no signal as a *hard* zero-view ("this name
earns exactly consensus") rather than as ignorance, and treats every view's
confidence as baked into its magnitude alone. Black–Litterman fixes both: a single
precision-weighted blend of the (zero, in residual space) prior and our views, where
prior precision comes from ``(τΣ)⁻¹`` and view precision from measured IC. Names we
never scored still get a real, nonzero posterior when they're correlated with a
name we *did* score (G&K ch. 11's "never zero the correlated forecasts") — the
propagation is the entire point.
...
- :func:`black_litterman_from_ic` — derives ``Ω`` from IC via the §3.2 calibration
  identity (:func:`view_variance`) and calls the primitive. This is what
  ``construct_portfolio`` calls.

Everything here is in **residual/active space** (spec 017): the prior mean is
exactly 0, not because our alphas happen to be benchmark-neutral, but because the
...
must be the *unshrunk* refined alphas (spec 020's `AlphaContext.level_shrink` left
off upstream) — Ω below already carries the full IC-uncertainty haircut, so shrinking
twice would halve it twice (spec 021 §4 hidden factor 1).
"""
...
@dataclass
class BLPosterior:
    """The BL posterior over Σ's full universe, plus the audit trail §3.3 wants."""
...
def view_variance(residual_vol: float, ic: float, t_eff: float) -> float:
    """The §3.2 calibration-forced view-error variance ``Ω_n = ω_n² / (T_eff²·IC²)``.

    Not G&K's naive ``ω²·(1−IC²)`` — that formula alone prices only "the signal
    isn't perfectly correlated with truth," and ignores that IC itself is
    *estimated* over a finite ``T_eff``. The calibration identity (§3.2: a single
    covered-name view, uninformative prior, pinned ``τ = 1/T_eff``, must reproduce
    the 020-shrunk alpha exactly — the ``g/(g+1)`` weight
    ...
    solving for ``Ω`` gives exactly ``ω²/(T_eff²·IC²)``. See the spec's §7 "write
    the one-page derivation first" open question — this docstring is that page.
...
def expected_single_view_weight(ic: float, t_eff: float) -> float:
    """The blend weight a lone covered-name view must land on (§3.2's identity).
...
def calibration_gap(posterior: "BLPosterior") -> Optional[float]:
    """For a single-view posterior, ``actual_weight − expected_weight`` (§3.2).
    ...
    else means Ω is mis-derived relative to 020's level shrink (double- or
    under-shrunk). Returns ``None`` when the posterior isn't single-view (§4 hidden
    factor 4 covers the K=N matrix-form generalization of this same identity) or
...
def black_litterman(
    ...
    """The BL update in residual/active space (§3.1):
    ...
    view — §4 hidden factor 5: exactly one view per name, 013 already
    decorrelated/combined upstream) are keyed by symbol; any symbol in ``views``
    absent from ``risk.symbols`` is ignored. Also computes the τ-sensitivity
    (posterior recomputed at ``τ/2`` and ``2τ``, §3.3).
    """
...
def black_litterman_from_ic(
    ...
    """:func:`black_litterman` with Ω derived from IC (§3.2) and τ pinned to
    ``1/T_eff`` (§3.1) — what ``construct_portfolio`` calls.
