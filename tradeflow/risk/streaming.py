...
:func:`tradeflow.risk.base.build_risk_matrix` (and the factor model that shares its
...
:class:`~tradeflow.data.store.ParquetBarStore` in date-chunks and accumulating sufficient
...
- :func:`streaming_sample_covariance` ↔ :func:`build_risk_matrix` with
  :class:`~tradeflow.risk.sample.SampleCovariance`.
- :func:`streaming_factor_risk_matrix` ↔ :func:`tradeflow.risk.factor.estimate_factor_model`
  (the structural ``Σ = X F Xᵀ + Δ``). Exposures stay a small ``N×K`` leaf computed by
  :func:`tradeflow.risk.exposures.build_factor_exposures`.
...
def _kept_and_blocks(
    ...
    from tradeflow.data import compute
...
def streaming_sample_covariance(
    ...
    *,
    ...
    Mirrors :func:`build_risk_matrix` with :class:`~tradeflow.risk.sample.SampleCovariance`
    (population covariance), accumulating cross-products chunk-by-chunk via
    :func:`tradeflow.data.compute.streaming_covariance`. Returns ``None`` when no name clears
...
def streaming_factor_risk_matrix(
    ...
    *,
    ...
    Mirrors :func:`tradeflow.risk.factor.estimate_factor_model`: factor returns are recovered
    ...
    leaf from :func:`tradeflow.risk.build_factor_exposures`) stay pandas; only the panel-wide
