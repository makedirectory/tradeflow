...
- **Shrinkage/sample** (:class:`~tradeflow.risk.base.RiskMatrix`): condition the diagonal,
  keep the LW-shrunk (or raw) correlation matrix fixed — :func:`condition_risk_matrix`.
- **Factor** (:class:`~tradeflow.risk.factor.FactorRiskMatrix`): condition *both*
...
review). This mirrors the streaming accumulator shape of :mod:`tradeflow.risk.streaming`
...
**The alpha/risk vol seam.** :func:`tradeflow.data.features.add_risk_features`
...
def _assemble_d_r_d(
    ...
    (:func:`tradeflow.risk.base.build_risk_matrix`) already gives thin-history names.
    """
...
def condition_risk_matrix(
    ...
    :class:`~tradeflow.risk.factor.FactorRiskMatrix` conditions ``factor_cov``/``specific_var``
...
class VolForecastEvaluation:
    ...
    (:func:`tradeflow.services.analysis.evaluate_conditional_risk`)."""
