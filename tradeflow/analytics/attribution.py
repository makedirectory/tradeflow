...
the service layer (:func:`tradeflow.services.analysis.compute_attribution`), mirroring
how :mod:`tradeflow.analytics.information` stays pure math while
...
from tradeflow.analytics.metrics import norm_cdf
...
def attribute_period(
    ...
    volatility/size, :func:`tradeflow.risk.exposures.build_factor_exposures`);
    ``beta_per_name`` = the canonical Σ-implied per-name beta
    (:meth:`~tradeflow.risk.base.RiskMatrix.implied_beta`, the one canonical β
...
def prior_weight_t0(min_obs: int, bars_per_period: float) -> float:
    ...
    (:func:`tradeflow.risk.base.build_risk_matrix`, default 60); at this attribution's
...
def years_to_significance(ir: float) -> float:
    ...
    precise :func:`tradeflow.analytics.information.ir_standard_error` includes an
