...
The refinement runs over a :class:`~tradeflow.data.panel.FeaturePanel`: a scorer
(:mod:`tradeflow.alphas.scorers`) fills the ``score`` column, :func:`refine_alpha` adds
``z`` and ``alpha`` via the identity ``alpha = sigma * IC * z``, and
:func:`panel_to_alphas` exports the ranked :class:`Alpha` rows.
"""

from tradeflow.alphas.base import (
    DEFAULT_IC,
...
from tradeflow.alphas.scorers import Scorer, scanner_scorer, signal_scorer, strategy_scorer
