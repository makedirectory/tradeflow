...
:mod:`tradeflow.alphas.base` is unit-testable in isolation and composable.
...
def winsorize(scores: pd.Series, lower: float = 0.025, upper: float = 0.975) -> pd.Series:
    ...
    step on thin universes (see :mod:`tradeflow.alphas.base`).
    """
    if scores.empty:
        return scores
...
def level_shrink_factor(ic: float, t_eff: float) -> float:
    ...
    a raw row count — see :func:`tradeflow.alphas.horizon.effective_sample_size`). Anchors:
