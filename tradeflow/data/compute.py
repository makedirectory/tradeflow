...
- **Polars ``LazyFrame``** for dataframe-style transforms — rolling-window indicators
  ...
  Nothing materializes until a terminal :func:`~tradeflow.data.edges.collect_streaming`.
...
3. **The pandas edge stays narrow.** Every crossing back to pandas routes through
   :mod:`tradeflow.data.edges`; this module returns Polars frames / numpy, never pandas.
...
The legacy pandas implementations (:mod:`tradeflow.indicators.indicators`,
:mod:`tradeflow.alphas.refine`, :mod:`tradeflow.risk.sample`) are kept as the equivalence
...
# Pandas (tradeflow/indicators) stays the per-symbol leaf and the equivalence oracle.
# --------------------------------------------------------------------------- #
...
# tradeflow/alphas/refine.py stays the equivalence oracle.
#
...
# instead of materializing the full T×N matrix. tradeflow/risk/sample.SampleCovariance
...
def streaming_covariance(
    ...
    panel — the same panel :func:`tradeflow.risk.base.build_return_panel` produces, but fed
    ...
    and *numerically* matches :class:`~tradeflow.risk.sample.SampleCovariance` to machine
    epsilon for the inputs it is contracted on — **return-scale, near-zero-mean** rows
    (what :func:`tradeflow.risk.base.build_return_panel` produces from ``pct_change``). It is
...
def sql_query(
    ...
    *,
    ...
    :func:`tradeflow.data.edges.to_pandas`.
