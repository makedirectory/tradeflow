...
Crucially it implements the *same* :class:`~tradeflow.data.scan.BarSource` contract as the
in-memory :class:`~tradeflow.data.scan.ClientBarSource`, so it is a **drop-in**: the alpha,
...
class ParquetBarStore:
    ...
    def partition_paths(
        ...
        The file list a DuckDB/Polars set-based query (:func:`tradeflow.data.compute.sql_query`)
    ...
    def scan_lazy(
        ...
        (:func:`tradeflow.data.edges.collect_streaming`). The leaf scan + pushdown stream; a
        downstream ``over(...)``/sort buffers (see :mod:`tradeflow.data.compute` on what is
